        ...
        for idx, (inp, gt) in enumerate(data_loader):
            ...
            for i in range(1, len(l_image_losses)):
                sum_losses = sum_losses + \
                             (l_image_losses[i].detach() - l_image_losses[i - 1].detach()) * (
                                         (l_image_losses[i].detach() - l_image_losses[i - 1].detach()) > 0).float()
            ...
            if idx % self.args.log_freq == 0:
                logger.log_info('step: [{0}][{1}/{2}]\tbatch-time: {meters[batch_time]:.3f}'.format(epoch + 1, idx,
                                                                                                    len(data_loader),
                                                                                                    meters=self.meters))
